...
def get_performance(driver, player_slug, player_id):
    ...
    try:
        header = driver.find_element(
            By.XPATH,
            "//div[@id='daily-projections']/div[3]/div[@class='fg-data-grid undefined with-selected-rows sort-disabled  ']/div[@class='table-wrapper-outer']/div[@class='table-wrapper-inner']/div[@class='table-scroll']/table/thead",
        )
    ...
    body = driver.find_element(
        By.XPATH,
        "//div[@id='daily-projections']/div[3]/div[@class='fg-data-grid undefined with-selected-rows sort-disabled  ']/div[@class='table-wrapper-outer']/div[@class='table-wrapper-inner']/div[@class='table-scroll']/table/tbody",
    )
...
def scrape_fangraphs(fangraph_roster):
    ...
    performance_dict = [
        get_performance(driver, y, x)
        for x, y in zip(
            fangraph_roster["FG MajorLeagueID"], fangraph_roster["name_slug"]
        )
    ]
    ...
    filtered_stats = [
        {key: val for key, val in sub.items() if key in STATS} for sub in filtered_dict
    ]
