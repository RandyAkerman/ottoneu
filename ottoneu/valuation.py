...
from constants import (
    VALUATION_DOWNLOAD_BUTTON,
    VALUATION_PITCHERS,
    VALUATION_BATTERS,
    VALUATION_OTTONEU_4x4_PRESET,
    VIBBER_GOOGLESHEET_ID,
    VIBBER_GOOGLESHEET_TAB,
)
...
def get_fangraphs_auction_values():
    ...
    for f in glob.glob(
        os.getenv("VALUATION_PATH") + "fangraphs-auction-calculator*.csv"
    ):
        os.remove(f)
    ...
    time.sleep(5)
    os.rename(
        os.path.join(os.getenv("VALUATION_PATH"), f"fangraphs-auction-calculator.csv"),
        os.path.join(
            os.getenv("VALUATION_PATH"), f"fangraphs-auction-calculator-batters.csv"
        ),
    )
    ...
    os.rename(
        os.path.join(os.getenv("VALUATION_PATH"), f"fangraphs-auction-calculator.csv"),
        os.path.join(
            os.getenv("VALUATION_PATH"), f"fangraphs-auction-calculator-pitchers.csv"
        ),
    )
...
def surplus_calculator(player_type):
    ...
    if player_type == "batters":
        ...
        valuation = pd.read_csv(
            os.path.join(
                os.getenv("VALUATION_PATH"),
                f"fangraphs-auction-calculator-pitchers.csv",
            ),
            usecols=[
                "IP",
                "mERA",
    ...
    compare_value = valuation.merge(
        roster, how="inner", left_on="PlayerId", right_on="FG MajorLeagueID"
    )
